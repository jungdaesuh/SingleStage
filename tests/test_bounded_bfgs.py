...
class BoundedBFGSTests(unittest.TestCase):
    def test_initial_evaluation_is_included_in_counts(self):
        def objective(_x):
            raise AssertionError("unexpected objective call")

        result = minimize_bounded_bfgs(
            objective,
            np.zeros(1),
            initial_value=0.0,
            initial_gradient=np.zeros(1),
    ...
    def test_steps_never_exceed_the_configured_radius(self):
        ...
        # Success is an INF-norm contract; downstream gates re-verify exactly
        # this, so a regression here silently breaks the driver's gate.
        self.assertLessEqual(np.linalg.norm(result.jac, ord=np.inf), 1.0e-8)
