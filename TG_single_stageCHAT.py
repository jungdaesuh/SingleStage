...
boozersurface = BoozerSurface(
    biotsavart, surface, label, targetlabel, constraint_weight,
...
#try:
    #not_self_intersecting = not surface.is_self_intersecting()      TG: Commented out this line due to missing ground package
...
# ==============================================================================
# STATE INITIALIZATION (Required for Script 2's tracking logic)
# ==============================================================================
if "run_dict" not in globals():
    run_dict = {}

# Initialize variables required by the new logic using Script 1's structures
...
run_dict["x_prev"]       = JF.x.copy()
run_dict["lscount"]      = 0
run_dict["failed_solves"] = 0  # Counter for consecutive failed Boozer solves
run_dict["it"]           = 0  # Iteration counter for the callback

def func(x):
    # Calculate and print step size change
    ...
    # Reset to the last accepted surface state before running the solver
    boozersurface.surface.x   = run_dict["surface_dofs"]
    boozersurface.res["iota"] = run_dict["iota"]
    boozersurface.res["G"]    = run_dict["G"]

    # Assign new degrees of freedom to the objective function
    JF.x = x
    
    # Run the Boozer surface solver
    res = boozersurface.run_code(run_dict["iota"], run_dict["G"])
    
    # Check solver success and catch potential self-intersection exceptions safely
    try:
        res_success = res['success']
        # If your surface object supports it, uncomment the line below:
        # not_self_intersecting = not boozersurface.surface.is_self_intersecting()
    ...
    if solve_success:
        ...
        # Increment consecutive failures. You can raise an exception here if it loops too long.
        run_dict["failed_solves"] += 1
        print(f"Consecutive failed Boozer solves: {run_dict['failed_solves']}")

        # Revert to last accepted optimization step values and invert gradient direction
        J = run_dict["J"]
        dJ = -run_dict["dJ"]
        boozersurface.surface.x   = run_dict["surface_dofs"]
        boozersurface.res["iota"] = run_dict["iota"]
...
def callback(x):
    # Reset line search count tracker for the next iteration step
    run_dict["lscount"] = 0
    
    # Cache the current valid configuration as the baseline for the next step
    run_dict["surface_dofs"] = boozersurface.surface.x.copy()
    run_dict["iota"]         = boozersurface.res["iota"]
    run_dict["G"]            = boozersurface.res["G"]
    run_dict["J"]            = JF.J()
    ...
    print(f"Callback (Iteration {run_dict['it']}): ")
    ...
    # Increment iteration tracking counter
    run_dict["it"] += 1

# Optimization parameters preserved from Script 1
maxiter = 150
gtol = 1e-2

# Swapped method to "L-BFGS-B" for speed, scaling, and memory efficiency
min_res = minimize(
    func, 
    JF.x.copy(), 
    jac=True, 
...
# Output directory and export setup preserved from Script 1
output_dir = "/burg-archive/home/tg2998/simsopt/examples/outputs/TG_single_stage_outputs"
savefile = os.path.join(output_dir, f"CHATCLUTCHoptimized_boozer_surface_{TF_a_cur}.json")
